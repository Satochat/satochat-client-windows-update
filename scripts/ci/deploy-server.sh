...
echo "Stopping containers..."
ssh "$DEPLOY_APPSERVER_SSH_HOST" "cd '$remoteTempDir' && docker-compose -f docker-compose.yml -f docker-compose.ci.prod.yml down" || exit 1

echo "Running containers..."
ssh "$DEPLOY_APPSERVER_SSH_HOST" "cd '$remoteTempDir' && docker-compose -f docker-compose.yml -f docker-compose.ci.prod.yml up -d" || exit 1
